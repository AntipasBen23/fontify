...
export class FontDetectionService {
  ...
  private detectFromVSCodeSettings(): DetectedFont[] {
    const fontFamily = this.workspace.getConfiguration('editor').get<string>('fontFamily');
  ...
  private async detectFromFiles(token?: vscode.CancellationToken): Promise<DetectedFont[]> {
    const fonts: DetectedFont[] = [];
    ...
    try {
      ...
      for (const file of cssFiles.slice(0, 20)) { // Limit for performance
        ...
        if (cssMatch) {
          cssMatch.forEach(match => {
            const fontNames = this.parseFontFamily(match.replace(/font-family\s*:\s*/i, ''));
            fonts.push(...fontNames.map(name => ({
              name,
              source: 'css' as const,
      ...
      // Tailwind config
      const tailwindFiles = await this.workspace.findFiles('tailwind.config.{js,ts}');
      for (const file of tailwindFiles) {
        if (token?.isCancellationRequested) break;
        
        const content = await fs.readFile(file.fsPath, 'utf8');
        const fontMatch = content.match(/fontFamily\s*:\s*\{[^}]*['"`]([^'"`]+)['"`]\s*:\s*\[([^\]]+)\]/g);
      ...
      // Package.json
      const packageFiles = await this.workspace.findFiles('**/package.json', '**/node_modules/**');
  ...
  private isSystemFont(font: string): boolean {
    const systemFonts = ['serif', 'sans-serif', 'monospace', 'arial', 'helvetica', 'times'];
