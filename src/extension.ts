...
async function detectFontsCommand() {
  try {
    ...
    if (fonts.length === 0) {
      vscode.window.showInformationMessage('✅ No custom fonts detected in your project');
      return;
    }

    const bySource = fonts.reduce((acc, font) => {
      if (!acc[font.source]) {
        acc[font.source] = [];
      }
    ...
    vscode.window.showErrorMessage('Failed to detect fonts. Check output for details.');
  }
}
    ...
    ...Object.entries(
      ...
      ...fonts.map(font => 
        `- **${font.name}**${font.filePath ? ` (${font.filePath})` : ''}${
          font.isInstalled !== undefined ? ` - ${font.isInstalled ? '✅ Installed' : '❌ Not installed'}` : ''
        }`
      ),
      ''
    ]).flat()
